...
import { MessageSquare, X, Send, Leaf, ImagePlus, Camera } from 'lucide-react';
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="fixed bottom-24 right-6 w-[350px] max-w-[calc(100vw-2rem)] z-40 rounded-lg shadow-xl overflow-hidden"
            initial={{ opacity: 0, y: 20, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 20, scale: 0.95 }}
            transition={{ duration: 0.2 }}
          >
            <div className={`flex flex-col h-[500px] max-h-[70vh] ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}>
              {/* Chat Header */}
              ...
              {isPhotoMode && (
                <div className="p-3 border-t border-gray-200 dark:border-gray-700">
                  <div className="flex justify-between items-center mb-2">
                    <h4 className="text-sm font-medium">Upload Plant Photo</h4>
                    <Button
                      variant="ghost"
                      size="sm"
